   ...
   if sw_current ~= last_sw then
      ...
      if sw_current == 0 then
        ...
        if previous_location ~= nil then
            ...
            gcs:send_text(MAV_SEVERITY.INFO, string.format("Restored previous location"))
        end
         gcs:send_text(MAV_SEVERITY.INFO, string.format("Switched to AUTO"))
      else
        previous_location = vehicle:get_previous_location()
        vehicle:set_mode(FLIGHT_MODE.LOITER)
         gcs:send_text(MAV_SEVERITY.INFO, string.format("Switched to LOITER"))
      end
   end
   return update,100
end
